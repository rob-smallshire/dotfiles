# This attempts to build a distribution directory for the pybind11 workshop.
#
...
WORKSHOP_NAME=sixty-north-software-engineering-for-data-scientists
PROJECT_NAME=gold_and_news
...
PROJECT_DIR=$DIST_DIR/$PROJECT_NAME
mkdir -p $PROJECT_DIR/
cp -R $GIT_CLONE/exercises/build/html $DIST_DIR/exercises
cp -R $GIT_CLONE/exercises/solutions $PROJECT_DIR/solutions


# Copy supporting materials from git repo
cp -R $GIT_CLONE/data $PROJECT_DIR
cp -R $GIT_CLONE/extra $PROJECT_DIR
...
cp -R $GIT_CLONE/project/* $PROJECT_DIR
